...
public class Player : MonoBehaviour, IKitchenObjectParent
{
    ...
    private void HandleMovement()
    {
        ...
        if (!canMove)
        {
            ...
            canMove = Mathf.Abs(moveDir.x) > 0.5f && !Physics.CapsuleCast(transform.position, transform.position + Vector3.up * playerHight, playerRadius, moveDirX, moveDistance);
            if (canMove) moveDir = moveDirX;
            else
            {
                //Attempt only  z movement
                Vector3 moveDirZ = new Vector3(0, 0, moveDir.z).normalized;
                canMove = Mathf.Abs(moveDir.z) > 0.5f && !Physics.CapsuleCast(transform.position, transform.position + Vector3.up * playerHight, playerRadius, moveDirZ, moveDistance);
